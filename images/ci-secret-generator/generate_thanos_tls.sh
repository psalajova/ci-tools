...
if [ "$#" -ne 3 ]; then
  echo "Usage: $0 <output-dir> <kubeconfig-path> <elb-hostname>"
  exit 1
fi
...
openssl req -new -key "${OUTPUT_DIR}/server.key" \
  -subj "/CN=thanos-receive-router" \
  -out "${OUTPUT_DIR}/server.csr" 2>/dev/null
openssl x509 -req -in "${OUTPUT_DIR}/server.csr" \
  -CA "${OUTPUT_DIR}/ca.crt" -CAkey "${OUTPUT_DIR}/ca.key" -CAcreateserial \
  -days 90 -sha256 \
  -extfile <(printf "subjectAltName=DNS:%s,DNS:thanos-receive-router-ci-metrics.ci-monitoring.svc.cluster.local" "${ELB_HOSTNAME}") \
  -out "${OUTPUT_DIR}/server.crt" 2>/dev/null

rm -f "${OUTPUT_DIR}/server.csr" "${OUTPUT_DIR}/ca.srl" "${OUTPUT_DIR}/ca.key"
